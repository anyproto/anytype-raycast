...
import { getCustomTypeIcon, getMaskForObject, getIconWithFallback, fetchWithTimeout, getFile } from './icon';
import { Image, Icon } from '@raycast/api';
import { ObjectLayout, IconFormat, ObjectIcon, RawType } from '../models';
...
describe('fetchWithTimeout', () => {
  ...
  it('should return undefined when fetch times out', async () => {
    vi.mocked(fetch).mockImplementation(() => new Promise((resolve) => {
      setTimeout(() => resolve({ ok: true } as any), 2000);
    }));

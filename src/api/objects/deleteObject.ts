...
export async function deleteObject(spaceId: string, objectId: string): Promise<SpaceObject | null> {
  ...
  return response ? await mapObject(response.payload.object) : null;
}
